...
Attempting to find a password match for a user:
...
Examining the file, it looks to a SAM file:
...
We could try to crack these hashes, but since they are NTLMv1 we can test these accounts with their hashes using "Pass the hash".
